  ...
  const nextStep = async (e) => {
    ...
    if (
      ...
      if (
        ...
        axios
          .post(USER_API_ENDPOINTS.SIGNUP_API, data)
          .then((res) => {
            console.log("response", res);
            // ----------------kirti--------------login part----------
            // // email not verified //its part of login now - kirti
            // if (res.data.msg === 'your unverified account already exists!') {
            //   // logic for sending regenerate verification email
            //   axios.post(`${apiUrl}/v3/api/regenerate-verification-email/customer/`, {
            ...
            // user already exists
            // else if (res.data.msg === 'User already exists as Customer please login for the same account type') {
            //   Swal.fire({
